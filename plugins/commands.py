...
from database.topdb import TopDB
...
movie_series_db = TopDB(DATABASE_URI)
verification_ids = {}
...
async def start(client:Client, message):
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener or is_third_shortener:
            ...
            buttons = [[
                InlineKeyboardButton(text="• ᴠᴇʀɪғʏ ɴᴏᴡ", url=verify),
                InlineKeyboardButton(text="ᴛᴜᴛᴏʀɪᴀʟ •", url=settings['tutorial'])
                ],[
    ...
    if data and data.startswith("allfiles"):
        ...
        delCap = "<b>This File is deleting automatically in {}. Forward in your Saved Messages..!</b>".format(len(files_to_delete), f'{FILE_AUTO_DEL_TIMER / 60} ᴍɪɴᴜᴛᴇs' if FILE_AUTO_DEL_TIMER >= 60 else f'{FILE_AUTO_DEL_TIMER} sᴇᴄᴏɴᴅs')
        afterDelCap = "<b>All your files have been successfully deleted.</b>".format(len(files_to_delete), f'{FILE_AUTO_DEL_TIMER / 60} ᴍɪɴᴜᴛᴇs' if FILE_AUTO_DEL_TIMER >= 60 else f'{FILE_AUTO_DEL_TIMER} sᴇᴄᴏɴᴅs')
    ...
    delCap = "<b>This File is deleting automatically in {}. Forward in your Saved Messages..!</b>".format(f'{FILE_AUTO_DEL_TIMER / 60} ᴍɪɴᴜᴛᴇs' if FILE_AUTO_DEL_TIMER >= 60 else f'{FILE_AUTO_DEL_TIMER} sᴇᴄᴏɴᴅs')
    afterDelCap = "<b>All your files have been successfully deleted.</b>".format(f'{FILE_AUTO_DEL_TIMER / 60} ᴍɪɴᴜᴛᴇs' if FILE_AUTO_DEL_TIMER >= 60 else f'{FILE_AUTO_DEL_TIMER} sᴇᴄᴏɴᴅs') 
    replyed = await message.reply(
        delCap,
...
async def set_shortner(c, m):
    ...
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/shortlink inshorturl 06b24eb6bbb025713cd522fb3f696b6d5de11354`</b>")
        return        
...
async def set_shortner_2(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/verify2 publicearn.com 06b24eb6bbb025713cd522fb3f696b6d5de11354`</b>")
        return
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://telegram.dog/Moviesflixop').json()
        if resp['status'] == 'success':
            SHORT_LINK = resp['shortenedUrl']
...
async def set_shortner_3(c, m):
    ...
    if len(m.text.split()) == 1:
        return await m.reply("<b>Use this command like this - \n\n`/verify3 publicearn.com 06b24eb6bbb025713cd522fb3f696b6d5de11354`</b>")
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await sts.delete()
    ...
    if not await is_check_admin(c, grp_id, userid):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.command) == 1:
        await m.reply_text("<b>ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴀᴅᴅ sʜᴏʀᴛɴᴇʀ & ᴀᴘɪ\n\nᴇx - `/verify3 mdiskshortner.link e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`</b>", quote=True)
        return
    try:
        URL = m.command[1]
        API = m.command[2]
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/codeflix_bots>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/verify3 mdiskshortner.link e7beb3c8f756dfa15d0bec495abc65f58c0dfa95`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
...
async def refer(bot, message):
    ...
    m=await message.reply_sticker("CAACAgUAAxkBAAEBGxpm-4YrnfCO3XWX58S-dJkkUSua7AAC7Q8AAnscKFUhu_6SMEM_7B4E")      
    await m.delete()
    reply_markup = InlineKeyboardMarkup(btn)
    await message.reply_photo(
...
async def support(bot, message):
    btn = [[
         InlineKeyboardButton("sᴜᴘᴘᴏʀᴛ", url="https://t.me/ragnarchats"),
         InlineKeyboardButton("ᴜᴘᴅᴀᴛᴇs", url="https://t.me/moviesflixop")
    ]]
...
async def donation(bot, message):
    btn = [[
        ...
        InlineKeyboardButton(text="sᴜᴘᴘᴏʀᴛ", url=f'https://t.me/RagnarChats'),
    ]]
